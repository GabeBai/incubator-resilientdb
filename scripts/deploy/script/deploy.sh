...
for ip in ${deploy_iplist[@]};
do
  run_one_cmd "mkdir -p /users/gabbai/${main_folder}/$idx" &
  folder="/users/gabbai/${main_folder}/$idx"
...
for ip in ${deploy_iplist[@]};
do
  echo "scp -i ${key} -r ${bin_path} ${output_path}/server.config ${output_path}/cert gabbai@${ip}:/users/gabbai/${main_folder}/$idx" 
